...
CONFIG = dict(
    ...
    train_batch_size=2,          # 3 -> 2. Can use 2 with videomae-large
    val_batch_size=2,            # 3 -> 2. Can use 2 with videomae-large
    num_workers_train=4,         # 6 -> 4
    num_workers_val=2,           # 4 -> 2

    # Optimisation
    learning_rate=1e-5,
    weight_decay=1e-4,
    epochs=3,
    accumulation_steps=12,       # 8 -> 12. Adjusted for batch_size=2 to maintain effective batch=24
    scheduler_T_max=2,
...
class VideoMAEClassifier(nn.Module):
    def __init__(self):
        super().__init__()
        # Changed from VideoMAEv2-giant to VideoMAEv2-large (fits in 10GB GPU)
        cfg = AutoConfig.from_pretrained("MCG-NJU/videomae-large", trust_remote_code=True)
        ...
        if CONFIG["pretrained_backbone"]:
            self.backbone = AutoModel.from_pretrained(
                "MCG-NJU/videomae-large", config=cfg, trust_remote_code=True
            ).to(device)
        else:
        ...
        with torch.no_grad():
            ...
            dummy = torch.rand(1, FRAME_COUNT, 3, IMG_SIZE, IMG_SIZE).to(device)
            outputs = self.backbone(pixel_values=dummy)
            feat_dim = outputs.last_hidden_state.shape[-1]
        self.dropout = nn.Dropout(0.1)
        self.classifier = nn.Linear(feat_dim, CONFIG["num_classes"]).to(device)

    def forward(self, x):
        # VideoMAE expects (B, T, C, H, W) not (B, C, T, H, W)
        # Input x is (B, T, C, H, W) from DataLoader
        x = x.float().to(device)
        outputs = self.backbone(pixel_values=x)
        # Get the last hidden state (pooled output)
        feats = outputs.last_hidden_state[:, 0]  # Take [CLS] token
...
def train():
    logger.info("="*60)
    logger.info("Begin training VideoMAE-Large (deterministic mode)")
    logger.info("="*60)
