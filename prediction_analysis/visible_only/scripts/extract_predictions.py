...
Usage:
    python prediction_analysis/visible_only/scripts/extract_predictions.py [--dataset VAL|ALL]

Outputs to prediction_analysis/visible_only/{dataset}/:
...
def main(dataset='ALL'):
    ...
    filtered_labels_path = os.path.join(PROJECT_ROOT, "prediction_analysis/visible_only/filtered_labels/final_matched_visible_only.csv")
    train_csv_path = os.path.join(PROJECT_ROOT, "dataset/train.csv")
    output_base = os.path.join(PROJECT_ROOT, f"prediction_analysis/visible_only/{dataset}")
