...
def plot_error_rate_by_scenario(df, level_col, title_suffix, output_name, output_dir):
    ...
    for _, row in error_stats.iterrows():
        ax2.annotate(row[level_col][:15] + '...' if len(row[level_col]) > 15 else row[level_col],
    ...
    ax2.set_title('Error Rate vs Sample Size\n(Bubble = scenario)', fontsize=14, fontweight='bold')
    ax2.spines['top'].set_visible(False)
    ax2.spines['right'].set_visible(False)
