"""
Predict and Aggregate from Validation Set (VAL)
...
⚠️  UNBIASED EVALUATION: Model never saw validation data during training.
    Use these results for reporting model performance.

Outputs to video_predictions_VAL/
"""
import os
import sys
import torch
...
def main():
    ...
    OUTPUT_DIR = os.path.join(PROJECT_ROOT, 'video_predictions_VAL')
    BATCH_SIZE = 4
    ...
    if not os.path.exists(MODEL_PATH):
        ...
        # Try alternative
        MODEL_PATH = os.path.join(PROJECT_ROOT, 'best_videomae_large_model.pth')
        if not os.path.exists(MODEL_PATH):
    ...
    seg_path = os.path.join(OUTPUT_DIR, 'segment_predictions.csv')
    ...
    aggregator = PredictionAggregator(
        ...
        original_labels_path=os.path.join(PROJECT_ROOT, 'dataset/train.csv'),
    ...
    print("\n" + "="*70)
    print("✅ EVALUATION COMPLETE (VALIDATION SET - UNBIASED)")
    print("="*70)
    print("\nResults saved to 'video_predictions_VAL/' directory:")
    print("  - aggregated_predictions_any.csv   ← Video-level predictions")
    print("  - segment_predictions.csv          ← Segment-level predictions")
    ...
    print("  - segment_vs_video_comparison.png")
    print("\nTo view results:")
    print("  cat video_predictions_VAL/detailed_report_any.txt")
