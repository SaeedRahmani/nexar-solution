"""Calculate model accuracy by scenario type - VALIDATION SET (VAL).

⚠️  UNBIASED EVALUATION: Uses validation set only (model never saw this data during training).
    Use these results for reporting model performance.

This script:
1. Loads final matched labels (from scenario_labels/final_matched_all.csv)
2. Merges with VALIDATION SET predictions only
3. Calculates per-scenario accuracy metrics (UNBIASED)
4. Generates visualization plots

Usage:
    python accuracy_per_scenario_VAL/accuracy_by_scenario.py

Outputs to accuracy_per_scenario_VAL/:
...
def main():
    ...
    predictions_path = os.path.join(PROJECT_ROOT, 'video_predictions_VAL/aggregated_predictions_any.csv')  # Validation set only (UNBIASED)
    output_dir = os.path.join(PROJECT_ROOT, 'accuracy_per_scenario_VAL')
    
    # Create output directory
    os.makedirs(output_dir, exist_ok=True)
    
    print("="*80)
    print("SCENARIO ANALYSIS - VALIDATION SET (VAL) - UNBIASED")
    print("="*80)
